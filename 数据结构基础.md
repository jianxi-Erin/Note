...
- 杂凑/散列/哈希方法：
    - 插入：选取某个函数，依次按key计算存储位置，并存储
    - 查询：用同一个函数对指定key计算地址，将k与地址内容比较，确定是否查找成功
...
- 冲突：不同key通过哈希函数计算的地址相同：
    ...
    - 具有相同函数的多个关键字
    - 冲突不可避免，只能尽可能减少
![拓扑](img/Screenshot_2024-11-27-11-32-39-744_tv.danmaku.bilibilihd.png)
