...
![图片](img/IMG_20241129_153339.png)
        - 性能分析：
            - 最好情况：O(n)
            - 最坏情况：O(n^2)
            - 平均时间复杂度：O(n^2)
    ...
    - 快速排序O(n log2 n)
        - 选取一个元素作为中心(pivot)
        - 其他元素与pivot比较,小的放左,大的放右
        - 形成左右两个子表，
        - 对两个子表重新选择中心，并依次规划调整
        - 直到每个子表只剩下一个元素，排序成功
![图片](img/Screenshot_2024-11-29-15-59-40-537_tv.danmaku.bilibilihd.png)
...
void bubble_sort(SqList *L){
    int m,i,j;
    flag=1//用于判断这趟元素是否有交换，
    RedType x;
    for(m=1;m<=n-1&& flag==1;m++){//总共需要m躺
        flag=0
        for(j=1,j<=n-m;j++){
            if(L.r[j].key>L.r[j+1].key){//发生逆序，则交换两个元素位置
                flag=1;//有交换，则赋为1，无交换则保持0
                x=L.r[j];
                L.r[j]=L.[j+1];
                L.[j+1]=x；
            }
...
| **排序算法** | **时间复杂度** | **排序思路** | **对数据的要求** | **稳定性** | **适用场景** | **不适合的场景** | **空间复杂度** | **例子** |
|--------------|----------------|--------------|------------------|------------|--------------|------------------|----------------|---------|
| **冒泡排序** | 最好：O(n) <br> 平均：O(n²) <br> 最坏：O(n²) | 比较相邻元素，交换位置 | 不需要额外的内存开销 | 稳定 | 小数据量，教育场景 | 大数据量或时间要求较高 | O(1) | 排序小规模数组或仅用于教学演示 |
| **选择排序** | 最好：O(n²) <br> 平均：O(n²) <br> 最坏：O(n²) | 每次选择最小（大）元素放到合适位置 | 不需要额外的内存开销 | 不稳定 | 小数据量，内存有限的场景 | 大数据量，时间要求较高 | O(1) | 排序小规模数组，或者内存空间有限的场景 |
| **插入排序** | 最好：O(n) <br> 平均：O(n²) <br> 最坏：O(n²) | 将当前元素插入到已排序的子数组中 | 数据部分有序时性能好 | 稳定 | 部分有序的数组，数据量较小 | 大数据量，时间要求较高 | O(1) | 排序几乎有序的数组，或者数据量较小时 |
| **快速排序** | 最好：O(n log n) <br> 平均：O(n log n) <br> 最坏：O(n²) | 分治法：选取基准元素，将数组分为两部分递归排序 | 适合大数据量的排序 | 不稳定 | 大数据量排序，尤其是随机数据 | 排序时对最坏情况敏感，空间较大 | O(log n) ~ O(n) | 排序大规模数组，或数据规模不小且随机分布 |
| **归并排序** | 最好：O(n log n) <br> 平均：O(n log n) <br> 最坏：O(n log n) | 分治法：递归分割数组并合并 | 需要额外的内存 | 稳定 | 适合大规模数据，尤其是外部排序 | 空间复杂度高，不适用于空间受限场景 | O(n) | 排序大数据或需要外部排序的场景 |
| **堆排序** | 最好：O(n log n) <br> 平均：O(n log n) <br> 最坏：O(n log n) | 使用堆数据结构，反复取出最大（最小）元素 | 适合数据量较大 | 不稳定 | 大数据量排序，优先考虑空间复杂度 | 不适合对稳定性有要求的场景 | O(1) | 排序大量数据或需要优先队列的场景 |
| **希尔排序** | 最好：O(n log n) <br> 平均：O(n^(3/2)) <br> 最坏：O(n²) | 基于插入排序，按间隔逐步减少 | 对间隔序列选择有要求 | 不稳定 | 中等数据量的排序，尤其是部分有序的数据 | 不适合时间要求严格的场景 | O(1) ~ O(n) | 排序较为接近有序的数组 |
| **计数排序** | 最好：O(n+k) <br> 平均：O(n+k) <br> 最坏：O(n+k) | 通过计数每个元素出现的次数然后重构数组 | 适合数据范围较小的情况 | 稳定 | 数据范围较小且值离散的排序 | 数据范围大，内存开销大 | O(k) | 排序年龄、成绩等整数数据 |
| **桶排序** | 最好：O(n+k) <br> 平均：O(n+k) <br> 最坏：O(n²) | 将元素分到不同的桶里，然后对每个桶排序 | 元素分布较为均匀的情况 | 稳定 | 数据均匀分布且值离散的排序 | 数据分布不均时效率低 | O(n+k) | 排序均匀分布的浮动数据，如浮点数 |
| **基数排序** | 最好：O(nk) <br> 平均：O(nk) <br> 最坏：O(nk) | 按位排序：从最低位到最高位排序 | 数据的范围和位数有限 | 稳定 | 数据范围较小且基于数字位数的排序 | 数据范围大时效率较低 | O(n+k) | 排序大量相同位数的整数（如身份证号、电话号码等） |
