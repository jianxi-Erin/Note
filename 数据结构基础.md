...
void get_next(const char* pattern, int* next) {
    int patternlen = strlen(pattern);  // 获取模式串的长度
    int prefix_len = 0;  // 当前最长相同前后缀的长度
    next[0] = 0;  // 初始化第一个元素，表示模式串的第一个字符无前缀
    int i = 1;  // 从模式串的第二个字符开始计算

    // 遍历模式串以构建 next 数组
    while (i < patternlen) {
        if (pattern[i] == pattern[prefix_len]) {  // 如果当前字符与前缀匹配
            prefix_len++;  // 更新当前前缀的长度
            next[i] = prefix_len;  // 设置 next 数组的值
            i++;  // 移动到下一个字符
        } else {
            if (prefix_len != 0) {  // 如果前缀长度不为零，查找上一个匹配的前缀
                prefix_len = next[prefix_len - 1];  // 回退到上一个部分匹配的位置
            } else {
                next[i] = 0;  // 当前字符与任何前缀都不匹配
                i++;  // 移动到下一个字符
            }
        }
    }
...
// 函数：KMP 算法实现模式匹配
int kmp_Index(const char* text, const char* pattern) {
    int textlen = strlen(text);  // 获取主串的长度
    int patternlen = strlen(pattern);  // 获取模式串的长度
    int next[patternlen];  // 创建用于存储 next 数组

    get_next(pattern, next);  // 构建 next 数组
    ...
    while (i < textlen) {
        ...
        if (j == patternlen) {  // 如果模式串已完全匹配
            return i - j;  // 返回匹配的起始索引
        } else if (i < textlen && text[i] != pattern[j]) {  // 如果当前字符不匹配
            if (j != 0) {
                j = next[j - 1];  // 使用 next 数组跳转到下一个匹配位置
            } else {
                i++;  // 如果 j 为 0，仅移动主串指针
            }
        }
