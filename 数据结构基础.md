...
#### 数组的定义
- 数组是一种数据结构，可以存储固定数量的相同类型的元素。数组在内存中是连续分配的，因此可以通过索引来快速访问数组中的元素。
##### 数组的基本概念
- 数组元素个数固定，定义后元素个数不会变化
- 数组中的数据元素有相同的数据类型
- 数组中每个元素都有唯一的下标对应。
- 数组是一种随机存储结构，可以随机存取数组中的任意数据元素。

- 一维数组:可以存储`array_size`个`type`类型的元素。
`type array_name[array_size]`
- 二维数组:二维数组是由多个一维数组组成的，可以看作一个矩阵。row_size是行数，col_size是列数。
`type array_name[row_size][col_size];`

#### 数组的类型定义、存储和实现
##### 一维数组
计算某个元素的存储地址(K为单个元素占用的空间):
`LOC(a[n])=LOC(a[1])+(n-1)*k`
##### 二维数组
计算某个元素前的存储地址：m*n为行列元素个数
以行计算:`LOC(a[i][j])=LOC(a[1][1])+((i-1)*n+(j-1))*k`
以列计算:`LOC(a[i][j])=LOC(a[1][1])+((i-1)*m(j-1))*k`
特殊矩阵：对称矩阵：a[i][j]=a[j][i]，压缩存储：只用存储主对角线和上/下三角的元素;;
特殊矩阵；上/下三角矩阵压缩存储：只用存储上/下三角元素，其他元素均为0
特殊矩阵：对角矩阵压缩存储：只存储对角线元素，其余为0
稀疏矩阵：零元素比较多，分布没有规律了吧：
    - 压缩方法(三元组存储法):只存储非零元素(i,j,a[i][j])




#### 广义表的定义
广义表：线性表的推广，是有限个元素的序列，其逻辑结构才用括号表示法
- 存储特点：
    - 可以包含原子元素（如整数、字符、符号等）。
    - 可以包含子表（即其他广义表）。
    - 可以为空表（即没有任何元素）。
- 长度:最外层元素个数
- 深度：括弧的深度，原子的深度为0，空表的深度为1
- 表头和表尾：表头为第一个元素a1，其- 余元素为表尾
