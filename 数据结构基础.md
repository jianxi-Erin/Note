...
#### 串的定义
##### 串的基本概念
串(字符串)：由零个或者多个字符组成的有限序列，是内容受限的线性表,结尾为‘\0’
...
串相等：两个串`长度相等`且`对应位置字符都相等`。所有空串都相等
...
#### 串的模式匹配
字符串查找：如果模式串是主串的子串，返回第一个字符位置，反之返回－1
##### Brute Force暴力匹配算法，通过逐个字符比对模式串与主串，直到找到匹配或遍历结束。
```c
#include <stdio.h>
#include <string.h>

// BF模式匹配算法
int BFPatternMatching(const char *text, const char *pattern) {
