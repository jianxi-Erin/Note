...
- 满二叉树:
    ...
    ```
- 完全二叉树:
    - 所有节点都尽可能填充，只有最底层最右边的节点可能不满，
    - 对于一个有n 个节点的完全二叉树，除最后一层外，其他层都完全填充。
完全二叉树的一个重要性质是其树高 h=向下取整(log 2(n))
    ```
    完全二叉树示例：
    ...
    ```
##### 二叉树的性质
- 性质1:二叉树第i层最多有2^(i-1)个结点
- 性质2：树的叶子结点数=树的总度－度数大于1的结点数－1
- 性质3:高度为h的m次树至多有(m^(h-1))/(m-1)个结点
- 性质4:具有n个结点的m叉树的最小高度为： 向上取整[log m (n(m-1)+1)]
- 
