...
- 性质2:深度为k的二叉树最多有2^k -1个结点(k>=1),最少k个结点
- 性质3:二叉树,叶子数为n0, 度为2的结点数为n2,则满足: n0=n2+1

- 更多性质:
  - 性质：树的叶子结点数=树的总度－度数大于1的结点数

  - 性质:高度为h的m次树至多有(m^(h-1))/(m-1)个结点
  
  - 性质:具有n个结点的m次树的最小高度为： 向上取整[log m (n(m-1)+1)] 
...
- 完全二叉树(满二叉树叶子却右):
    ...
    - 性质4 : n个结点的完全二叉树的树高度 h=向下取整[log 2(n)]+1
...
- 优点存储简单
![图片](img/IMG_20241108_214535.png)
![图片](img/IMG_20241108_215355.png)
- 缺点右单支树需要存储大量的0,空间浪费
![图片](img/IMG_20241108_221023.png)
```c
#define MAXSIZE 100
typedef int ElemType;
